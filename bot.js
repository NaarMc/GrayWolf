...
client.on("message", message => { 
  if(message.content.startsWith("-apply")) {
        if(!message.channel.guild) return;
                if(message.author.bot) return;
        let channel = message.guild.channels.find("name", "ا̍ڸــٿــڦــدٻۧــمۘ")
...
**
『-bc1 ====> رسالة لجميع اعضاء السيرفر
『-bcrole ====> رسالة لرتبة معينة
...
『-لقفل الروم ====> قفل الروم
『-لفتح الروم ====> فتح الروم
『-ct <name> ====> انشاء شات
...
client.on('message', message => {
    ...
    .addField('**💬 channels**',`**[ ${message.guild.channels.filter(m => m.type === 'text').size} ] Channel **`,true)[ ${message.guild.channels.filter(m => m.type === 'voice').size} ] Channel ** `,true)
